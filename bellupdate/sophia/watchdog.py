...
###
 ...
 #		Python watchdog for the "sophia" bell management system
...
while (1) :
	...
	if not ('demone.py' in output):
		if (prev_demone == 0) :
			...
			print time.strftime("[%H:%M:%S %d/%m/%Y") + "] Watchdog: sophia DAEMON CRASHED, setting led color to red"
			command = "logger " + time.strftime("[%H:%M:%S %d/%m/%Y") + "] Watchdog: sophia DAEMON CRASHED, setting led color to red";
			os.system(command)
		set_led_color("red")
	else :
		prev_demone = 0
